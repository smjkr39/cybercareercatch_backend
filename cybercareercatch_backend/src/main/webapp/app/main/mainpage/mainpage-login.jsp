...
@
keyframes answerModalPop {from { transform:translateY(10px)scale(0.97);
	opacity: 0;
}
...
<body>
	...
	<div class="mainpage-main-container"
		...
		<div class="mainpage-section-searchjob">
			...
			<div
				...
				답변에 대한 결과는 회원만 마이페이지에서 확인 가능합니다.</div>
					...
					<c:when test="${not empty mainCompanyList}">
						<c:forEach var="company" items="${mainCompanyList}">
							...
							<a
								...
								src="${companyImgSrc}" alt="${company.companyName}"
				...
				<div class="mainpage-community-calendarcard">
					<div class="mainpage-community-calendarhead">
						<div>
							...
							<div class="mainpage-community-calendardesc">등록 최신순 박람회 3개를
								확인해보세요.</div>
						</div>
					...
					<div class="expo-preview-wrap-force">
						<c:choose>
									...
									<div class="expo-preview-card-force"
										...
										<div class="expo-preview-info-force">
											<div class="expo-preview-row-force">
												<span class="expo-preview-badge-force">일정</span> <span
													class="expo-preview-text-force"> <c:out
														value="${expo.startDate}" /> ~ <c:out
														value="${expo.endDate}" />
												</span>
											</div>

											<div class="expo-preview-row-force">
												<span class="expo-preview-badge-force">장소</span> <span
													class="expo-preview-text-force"> <c:out
														value="${expo.location}" />
												</span>
											</div>
							...
							<c:otherwise>
								<div class="expo-preview-empty-force">현재 등록된 박람회 일정이 없습니다.
								</div>
							</c:otherwise>
						</c:choose>
					</div>
	...
	<!-- 1000자 제한 모달 -->
	<div id="mainpage-answer-length-modal" class="mainpage-answer-modal"
		aria-hidden="true">
		<div class="mainpage-answer-modal-box" role="dialog" aria-modal="true"
			aria-labelledby="mainpage-answer-modal-title">
			<div class="mainpage-answer-modal-head">
				<div id="mainpage-answer-modal-title"
					class="mainpage-answer-modal-title">안내</div>
				<button type="button" id="mainpage-answer-modal-close"
					class="mainpage-answer-modal-close" aria-label="닫기">×</button>
			</div>
			<div id="mainpage-answer-modal-message"
				class="mainpage-answer-modal-body">1000자 이내로 입력해주세요.</div>
			<div class="mainpage-answer-modal-foot">
				<button type="button" id="mainpage-answer-modal-confirm"
					class="mainpage-answer-modal-confirm">확인</button>
			</div>
		</div>
	</div>
